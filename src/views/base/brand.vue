...
<template>
    <div class="page-watch-list">
        <div style="margin:0 0 20px 0;position: relative">
            <Input search enter-button placeholder="输入品牌关键字搜索" v-model="keyWords" style="width: 300px" @on-search="search"/>
            <Button type="primary" style="position: absolute;right:20px;top:0" @click="value3=true;actionType='new'">新建品牌</Button>
        </div>
        ...
        <Drawer
                :title="actionType==='new'?'新建品牌':'编辑品牌'"
                v-model="value3"
                width="720"
                :mask-closable="false"
                :styles="styles"
        >
            <Form :model="formData" :rules="ruleCustom" ref="formCustom" :label-width="80">
                <Row>
                    <Col span="24">
                        <FormItem label="品牌名字" :prop="'brand'" required>
                            <Input v-model="formData.brand" placeholder="请输入品牌名字" />
                        </FormItem>
                    </Col>
                </Row>
                <Row>
                ...
                <!--<div v-if="actionType=== 'new'" style="margin-bottom: 20px ;">-->
                    ...
                    <!--<p class="tips" style="margin-bottom: 20px ; line-height: 26px; color: #999;">-->
                        <!--在下面的文本框输入多个字段并用 <b style="color: red">/</b> 分割可以实现一次性创建多个品牌，比如"百达翡丽/卡西欧/浪琴",注意不能添加系统已有的品牌，若当前文本框有值，则优先创建文本框的字段-->
                    <!--</p>-->
                            ...
                            return h('div', [
                                h('Button', {
                                    ...
                                    on: {click: () => {
                                        ...
                                        this.value3 = true;
                                        this.currentBrand = params.row;
                                        this.formData["brand"] = params.row["brand"];
                                    ...
                                    on:{
                                        click: () => {
                                            this.$Modal.confirm({
                                                render:(h)=>{
                                                    return h(
                                                        "p",{},[
                                                            ...
                                                            h(
                                                                "strong",{style:{fontWeight:"bold"}},params.row.brand
                                                            ),
                                                            h(
                                                                "span"," ，确定吗?"
                                                            )
                                                ...
                                                onOk:async()=>{
                                                    await this.toggleStatus(params.row.brandId,params.row.status === "Y"?"N":"Y");
                                                }
                                            });
                                        }
                                    }
                ...
                this.$refs["formCustom"].validate(async(success)=>{
                    ...
                    if(success){
                        ...
                        if(this.actionType !== "new"){
                            obj["brandId"] = this.currentBrand["brandId"];
                        }
                        try{
                            await baseServices[this.actionType==="new"?"create":"update"]("brand",obj);
                            ...
                            this.$Notice.success({
                                title:`${this.actionType==="new"?"新建":"编辑"}品牌成功!`
                            });
                            this.fetchData();
                        }catch (e){
                            this.submitting = false;
